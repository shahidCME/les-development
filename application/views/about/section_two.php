    ...
    <section class="wrapper">
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <!--breadcrumbs start -->
                <ul class="breadcrumb">
                    <li class="active"><a href=""><i class="fa fa-home"></i> <a href="<?php echo base_url().'admin/index'; ?>">Home</a> / testimonials</a></li>
                </ul>
                <!--breadcrumbs end -->
            </div>
        </div>
                        ...
                        <div class="adv-table">
                            <div id="example_wrapper" class="dataTables_wrapper form-inline" role="grid">
                                <div class="panel-body padding-zero" style="float: right">
                                    <a href="<?=base_url().'admins/about/about_section_two/add'?>" class="btn btn-primary">Add </a>
                                    <a href="#" id="delete_user" class="btn btn-danger">Delete </a>
                                </div>
                                <table class="display table table-bordered table-striped dataTable" id="section_two"
                                    ...
                                    <tbody role="alert" aria-live="polite" aria-relevant="all">
                                      <?php foreach ($section_two_data as $key => $value) { ?>
                                          <tr>
                                            ...
                                            <td><div id="" class="" style="width: 150px;height: 150px; margin-top: 20px; "><img id="ContentImage" src="<?=base_url().'public/uploads/about/'.$value->image ?>" height="100%" width="100%"></div></td>
                                            <td><?=$value->name?></td>
                                            <td><?=$value->designation?></td>
                                            <td><?=$value->content?></td>
                                            <td> <a href="<?php echo base_url() . 'admins/about/about_section_two/edit/'.$this->utility->safe_b64encode($value->id); ?>" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
    ...
    function single_delete(value) {
        ...
        bootbox.confirm("Are you sure you want to delete ?" , function (confirmed) {
            if (confirmed == true) {
                var id = value;
                $.ajax({
                    url: '<?php echo base_url().'admins/about/about_section_two/removeRecord'; ?>' ,
                    type:'post',
                    dataType:'json',
                    data: {
                        id: id
     ...
     $('#delete_user').click(function() {
        ...
        bootbox.confirm("Are you sure you want to delete ?" , function (confirmed) {
            if (confirmed == true) {
                ...
                $.ajax({
                    url: '<?php echo base_url().'admins/about/about_section_two/multipleDelete'; ?>',
                    dataType:'Json',
                    data: { ids: ids},
                    success: function(data) {
                        if(data.status == 1) {

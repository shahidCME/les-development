                        ...
                        <div class="adv-table">
                            <div id="example_wrapper" class="dataTables_wrapper form-inline" role="grid">
                                <div class="panel-body padding-zero" style="float: right">
                                    <a href="<?=base_url().'admins/faq/add'?>" class="btn btn-primary">Add </a>
                                ...
                                <table class="display table table-bordered table-striped dataTable" id="faq_customer_support"
                                    ...
                                    <tr role="row">
                                        ...
                                        <th class="sorting" role="columnheader" tabindex="0" aria-controls="example"
                                            ...
                                            style="width: 200px;">Questions
                                        </th>
                                        <th class="sorting" role="columnheader" tabindex="0" aria-controls="example"
                                            rowspan="1" colspan="1"
                                            aria-label="Platform(s): activate to sort column ascending"
                                            style="width: 200px;">Answer
                                        </th>                                      
                                        <th class="sorting" role="columnheader" tabindex="0" aria-controls="example"
                                            rowspan="1" colspan="1"
                                            aria-label="Rendering engine: activate to sort column ascending"
                                     ...
                                     <tbody role="alert" aria-live="polite" aria-relevant="all">
                                      <?php foreach ($faq as $key => $value) { ?>
                                          <tr>
                                            ...
                                            <td><?=$value->answer?></td>
                                            <td> <a href="<?php echo base_url() . 'admins/faq/edit/'.$this->utility->safe_b64encode($value->id); ?>" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
    ...
    function single_delete(value) {
        ...
        bootbox.confirm("Are you sure you want to delete ?" , function (confirmed) {
            if (confirmed == true) {
                var id = value;
                $.ajax({
                    url: '<?php echo base_url().'admins/faq/removeRecord'; ?>' ,
                    type:'post',
                    dataType:'json',
                    data: {
                        id: id
     ...
     $('#delete_user').click(function() {
        ...
        bootbox.confirm("Are you sure you want to delete ?" , function (confirmed) {
            if (confirmed == true) {
                ...
                $.ajax({
                    url: '<?php echo base_url().'admins/faq/multi_delete'; ?>',
                    data: { ids: ids },
                    dataType: 'json',
                    success: function(data) {
                        if(data.status == 1) {

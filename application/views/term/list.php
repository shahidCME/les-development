                        ...
                        <div class="adv-table">
                            <div id="example_wrapper" class="dataTables_wrapper form-inline" role="grid">
                                <div class="panel-body padding-zero" style="float: right">
                                    <a href="<?=base_url().'admins/term/add'?>" class="btn btn-primary">Add </a>
                                     ...
                                     <tbody role="alert" aria-live="polite" aria-relevant="all">
                                      <?php foreach ($getPrivacy as $key => $value) { ?>
                                          <tr>
                                            ...
                                            <td> <a href="<?php echo base_url() . 'admins/term/edit/'.$this->utility->safe_b64encode($value->id); ?>" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
    ...
    function single_delete(value) {
        ...
        bootbox.confirm("Are you sure you want to delete ?" , function (confirmed) {
            if (confirmed == true) {
                var id = value;
                $.ajax({
                    url: '<?php echo base_url().'admins/term/removeRecord'; ?>' ,
                    type:'post',
                    dataType:'json',
                    data: {
                        id: id
     ...
     $('#delete_user').click(function() {
        ...
        bootbox.confirm("Are you sure you want to delete ?" , function (confirmed) {
            if (confirmed == true) {
                ...
                $.ajax({
                    url: '<?php echo base_url().'admins/term/multi_delete'; ?>',
                    data: { ids: ids },
                    dataType:'json',
                    success: function(data) {
                        if(data.status == 1) {

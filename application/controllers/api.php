        ...
        if($login_type == '0'){
            if(isset($_POST['email']) && isset($_POST['phone']) && isset($_POST['lname']) ){
                ...
                if ($email_query->num_rows > 0) {
                    ...
                    $datas['message'] = $this->load->view('registration_mail', $datas, true);
...
//                }
        ...
        elseif($login_type == '3'){
           ...
           if($apple_query->num_rows > 0){
            ...
            $data = array(
                'id' => $apple_query->id,
                'fname' => $apple_query->fname,
                'lname' => $apple_query->lname,
                'email' => $apple_query->email,
                'phone' => $apple_query->phone,
                'login_type' => $apple_query->login_type,
                'cart_item' => $total_count[0]->cart_items,
                'notification_status'=>$notification_status,
            );
            ...
            if($login_check_query -> num_rows > 0){
                ...
                if($login_check_result['email_verify'] == '1'){
                   ...
                   $data = array(
                    ...
                    'cart_item' => $total_count[0]->cart_items,
                    'notification_status' =>$notification_status,
                    'mobile_verify' =>$login_check_result['is_verify']
                );
                   $this->this_model->update_device($login_check_result,$_POST);
    ...
    elseif($login_type == '2'){
        ...
        if($apple_result['login_type'] == '3'){
            ...
            if($apple_result['apple_token_id'] == $app_id){
                ...
                $data = array(
                    'id' => $apple_result['id'],
                    'fname' => $apple_result['fname'],
                    'lname' => $apple_result['lname'],
                    'email' => $apple_result['email'],
                    'phone' => $apple_result['phone'],
                    'login_type' => $apple_result['login_type']
                );
    ...
    if(isset($_POST['email'])) {
        ...
        if($result['total'] > 0){
            ...
            $config = Array(
                ...
                //Mail Send
            $from_email = 'test.cmexpertise@gmail.com';
...
public  function user_details(){
    ...
    if(isset($_POST['id'])){
        ...
        $vendorData['selfPickUp'] = $vendor_result[0]->selfPickUp;
        $vendorData['publish_key'] = ($p_method[0]->publish_key == null) ? '' : $p_method[0]->publish_key;
...
public function product_list()
{
    ...
    if(isset($_POST['category_id']) && isset($_POST['vendor_id']) ){
        ...
        $result_count = $this->db->query("SELECT p.* FROM `product` as p 
            ...
            WHERE p.status != '9' AND p.vendor_id = '$vendor_id' AND w.quantity >= '1'  AND w.discount_price != '' AND w.status != '9' AND p.category_id = '$category_id' AND p.subcategory_id = '$subcategory_id' GROUP BY p.id ORDER BY CAST(w.discount_price AS DECIMAL(10,2))");
...
//            echo $this->db->last_query();exit;
        ...
        $query = $this->db->query("SELECT p.* FROM `product` as p 
            LEFT JOIN product_weight as w ON w.product_id = p.id
            WHERE p.status != '9' AND p.vendor_id = '$vendor_id' AND w.discount_price != '' AND w.quantity >= '1' AND w.status != '9' AND p.category_id = '$category_id' AND p.subcategory_id = '$subcategory_id' GROUP BY p.id ORDER BY CAST(w.discount_price AS DECIMAL(10,2)) ASC LIMIT $limit OFFSET $cal");
        $result = $query->result();
...
//            exit;
        ...
        if($query->num_rows() > 0){
            ...
            foreach($result as $value => $row)
            {
                ...
                $weightcount = count($product_weight_result);
                foreach ($product_weight_result as $r  =>$pro_weight) {
                    ...
                    $res = $this->this_model->check_qnt($product_weight_id);
                    ...
                    if(isset($_POST['user_id']) && $_POST['user_id'] != ''){
                        ...
                        $my_cart_query = $this->db->query("SELECT quantity FROM my_cart WHERE product_id = '$product_id'AND product_weight_id = '$product_weight_id' AND  status != '9' AND vendor_id = '$vendor_id' AND user_id = '$user_id'");
                    ...
                    { 
                      if(isset($_POST['device_id']) ){
                       ...
                       $my_cart_query = $this->db->query("SELECT quantity FROM my_cart WHERE product_id = '$product_id' AND product_weight_id = '$product_weight_id' AND  status != '9' AND vendor_id = '$vendor_id' AND user_id = '0' AND device_id = '$device_id'");
                       $my_cart_result = $my_cart_query->row_array();
            ...
            $data = array(
                ...
                'variant_image' => base_url().'public/images/product_image/'.$product_image_result[0]->image,
            );
...
public function add_to_cart_old(){
    ...
    if(isset($_POST['product_id']) && isset($_POST['weight_id']) && isset($_POST['quantity'])){
...
public function add_to_cart_devlop(){
    ...
    if(isset($_POST['product_id']) && isset($_POST['weight_id']) && isset($_POST['product_weight_id']) && isset($_POST['quantity'])&& isset($_POST['vendor_id']) ){
     ...
     if(isset($_POST['user_id']) && $user_id != 0 && isset($_POST['device_id'])){

        $pro_available_query = $this->db->query("SELECT * FROM my_cart WHERE (device_id = '$device_id' OR user_id = '$user_id') AND product_id = '$product_id'  AND vendor_id = '$vendor_id' AND  product_weight_id = '$product_weight_id' AND weight_id = '$weight_id'");

        $product_weight_query = $this->db->query("SELECT * FROM product_weight WHERE id = '$product_weight_id'  AND vendor_id = '$vendor_id' AND status != '9'");
        $product_weight_result = $product_weight_query->row_array();
        $actual_price = $product_weight_result['price'];
        $actual_quantity = $product_weight_result['quantity'];
        $discount_per = $product_weight_result['discount_per'];
        ...
        $dt_updated = strtotime(date('Y-m-d H:i:s'));
        $this->db->query("UPDATE my_cart SET device_id = '$device_id', user_id = '$user_id', product_id = '$product_id', weight_id = '$weight_id', quantity = '$quantity', calculation_price = '$calculation_price',  dt_updated = '$dt_updated' WHERE product_id = '$product_id' AND product_weight_id = '$product_weight_id'  AND vendor_id = '$vendor_id' AND weight_id = '$weight_id' AND (device_id = '$device_id' OR user_id = '$user_id') ");

        $result_count = $this->db->query("SELECT COUNT(*) as total  FROM my_cart as mc WHERE (mc.device_id= '$device_id' OR mc.user_id= '$user_id')  AND mc.vendor_id = '$vendor_id' AND product_weight_id = '$product_weight_id' AND mc.status != '9' ORDER BY mc.id DESC");
        $row_count = $result_count->result();
        $total_count = $row_count[0]->total;

        $my_cart_price_query = $this->db->query("SELECT SUM(calculation_price) as total_price,quantity,actual_price from my_cart WHERE status != '9'  AND vendor_id = '$vendor_id' AND product_weight_id = '$product_weight_id' AND (device_id = '$device_id' OR user_id = '$user_id')");
        $my_cart_price_result = $my_cart_price_query->row_array();
        ...
        if($pro_available_query -> num_rows > 0){
            ...
            $this->db->query("UPDATE my_cart SET device_id = '$device_id', user_id = '$user_id', product_id = '$product_id', weight_id = '$weight_id', quantity = '$quantity', calculation_price = '$calculation_price',  dt_updated = '$dt_updated' WHERE  product_id = '$product_id'  AND vendor_id = '$vendor_id' AND weight_id = '$weight_id' AND product_weight_id = '$product_weight_id' AND (device_id = '$device_id' OR user_id = '$user_id') ");

            $setresponce = $this->db->query("SELECT * FROM my_cart WHERE (device_id = '$device_id' OR user_id = '$user_id')  AND vendor_id = '$vendor_id' AND status !='9'");
            ...
            $my_cart_price_query1 = $this->db->query("SELECT SUM(calculation_price) as total_price from my_cart WHERE status != '9'  AND vendor_id = '$vendor_id' AND (device_id = '$device_id' OR user_id = '$user_id')");
            $my_cart_price_result1 = $my_cart_price_query1->row_array();

            $result_count1 = $this->db->query("SELECT COUNT(*) as total  FROM my_cart as mc WHERE (mc.device_id= '$device_id' OR mc.user_id= '$user_id')  AND mc.vendor_id = '$vendor_id' AND mc.status != '9' ORDER BY mc.id DESC");
            $row_count1 = $result_count1->result();
            $total_count1 = $row_count1[0]->total;

            $setresponce = $this->db->query("SELECT * FROM my_cart WHERE device_id = '$device_id'  AND vendor_id = '$vendor_id' AND user_id = '$user_id' AND status !='9'");
            $setresponce = $setresponce->result(); 
    ...
    elseif($device_id && $device_id != ''){
        ...
        $pro_available_query = $this->db->query("SELECT * FROM my_cart WHERE device_id = '$device_id'  AND vendor_id = '$vendor_id' AND  product_id = '$product_id' AND product_weight_id = '$product_weight_id' AND weight_id = '$weight_id'");
        ...
        $where_cart1 = array('vendor_id' => $vendor_id,'product_weight_id'=>$product_weight_id, 'product_id' => $product_id, 'weight_id' => $weight_id, 'device_id' => $device_id);
        $data1 = array(
            'device_id' => $device_id,
            'product_id' => $product_id,
            'weight_id' => $weight_id,
        ...
        $result_count = $this->db->query("SELECT COUNT(*) as total  FROM my_cart as mc WHERE mc.device_id= '$device_id'  AND mc.vendor_id = '$vendor_id' AND mc.status != '9' ORDER BY mc.id DESC");
        $row_count = $result_count->result();
        $total_count = $row_count[0]->total;

        $my_cart_price_query = $this->db->query("SELECT SUM(calculation_price) as total_price from my_cart WHERE status != '9'  AND vendor_id = '$vendor_id' AND device_id = '$device_id'");
        $my_cart_price_result = $my_cart_price_query->row_array();

        if($pro_available_query -> num_rows > 0){

            $where_cart = array('vendor_id' => $vendor_id,'product_weight_id'=>$product_weight_id,'product_id' => $product_id, 'weight_id' => $weight_id, 'device_id' => $device_id);
            ...
            $setresponce = $this->db->query("SELECT * FROM my_cart WHERE device_id = '$device_id'  AND vendor_id = '$vendor_id' AND status !='9'");
            $setresponce = $setresponce->result(); 
            ...
            $result_count1 = $this->db->query("SELECT COUNT(*) as total  FROM my_cart as mc WHERE mc.device_id= '$device_id'  AND mc.vendor_id = '$vendor_id' AND mc.status != '9' ORDER BY mc.id DESC");
            $row_count1 = $result_count1->result();
            $total_count1 = $row_count1[0]->total;

            $my_cart_price_query1 = $this->db->query("SELECT SUM(calculation_price) as total_price from my_cart WHERE status != '9'  AND vendor_id = '$vendor_id' AND device_id = '$device_id'");
            $my_cart_price_result1 = $my_cart_price_query1->row_array();
    ...
    elseif ($user_id && $user_id != ''&& $user_id != 0){

        $pro_available_query = $this->db->query("SELECT * FROM my_cart WHERE user_id = '$user_id'  AND vendor_id = '$vendor_id' AND product_id = '$product_id' AND weight_id = '$weight_id'");

        $product_weight_query = $this->db->query("SELECT * FROM product_weight WHERE product_id = '$product_id'  AND vendor_id = '$vendor_id' AND weight_id = '$weight_id' AND status != '9'");
        $product_weight_result = $product_weight_query->row_array();
        $actual_price = $product_weight_result['price'];
        $actual_quantity = $product_weight_result['quantity'];
        $discount_per = $product_weight_result['discount_per'];
        ...
        $where_cart1 = array( 'vendor_id' => $vendor_id, 'product_id' => $product_id, 'weight_id' => $weight_id, 'user_id' => $user_id);
        $data1 = array(
            'user_id' => $user_id,
            'product_id' => $product_id,
            'weight_id' => $weight_id,
        ...
        if($pro_available_query -> num_rows > 0){
            ...
            $where_cart = array('vendor_id' => $vendor_id,'product_id' => $product_id, 'weight_id' => $weight_id, 'user_id' => $user_id);
            ...
            $setresponce = $this->db->query("SELECT * FROM my_cart WHERE user_id = '$user_id'  AND vendor_id = '$vendor_id' AND status !='9'");
            $setresponce = $setresponce->result(); 
            ...
            $result_count1 = $this->db->query("SELECT COUNT(*) as total  FROM my_cart as mc WHERE mc.user_id= '$user_id'  AND mc.vendor_id = '$vendor_id' AND mc.status != '9' ORDER BY mc.id DESC");
            $row_count1 = $result_count1->result();
            $total_count1 = $row_count1[0]->total;

            $my_cart_price_query1 = $this->db->query("SELECT SUM(calculation_price) as total_price from my_cart WHERE status != '9'  AND vendor_id = '$vendor_id' AND user_id = '$user_id'");
            $my_cart_price_result1 = $my_cart_price_query1->row_array();
...
public function my_cart_old(){
    ...
    $discount_price_total= 0;
    if(isset($device_id) && $device_id != '' && isset($user_id) && $user_id != '' && $user_id != 0 ){
        ...
        $my_cart_query = $this->db->query("SELECT mc.*  FROM my_cart as mc WHERE mc.status != '9' AND (mc.device_id = '$device_id' OR mc.user_id = '$user_id') ORDER BY mc.id DESC ");
        $my_cart_result = $my_cart_query->result();
        ...
        if($my_cart_query -> num_rows > 0){
            ...
            foreach($my_cart_result as $row)
            {
                ...
                $discount_price_total = ($product_actual_price*$row->quantity)-$row->calculation_price
            ...
            $response["count"] = $total_count;
            $response["actual_price_total"] = $actual_price_total;
    ...
    elseif(isset($device_id) && $device_id != ''){
        ...
        $my_cart_query = $this->db->query("SELECT mc.*  FROM my_cart as mc WHERE mc.device_id = '$device_id' AND mc.status != '9' ORDER BY mc.id DESC ");
        $my_cart_result = $my_cart_query->result();
    ...
    elseif (isset($user_id) && $user_id != ''){
        ...
        $my_cart_query = $this->db->query("SELECT mc.*  FROM my_cart as mc WHERE mc.user_id = '$user_id' AND mc.status != '9' ORDER BY mc.id DESC");
        $my_cart_result = $my_cart_query->result();
        ...
        if($query->num_rows > 0){
            ...
            foreach($result as $row)
            {
                ...
                $product_weight_query = $this->db->query("SELECT pw.*, p.name as product_name, p.image as product_image, w.name as product_weight_name FROM product_weight as pw 
                    ...
                    // echo $this->db->last_query();exit;
                $product_weight_result = $product_weight_query->row_array();
                    // print_r($product_weight_result);exit;
                $product_unit = $product_weight_result['weight_no'];
                $product_weight_name = $product_weight_result['product_weight_name'];
                $data['product_unit'] = $product_unit.' '.$product_weight_name;
                ...
                $discount_price_total = ($row->actual_price*$row->quantity)-$row->calculation_price
                +$discount_price_total;
...
public function user_address_add(){
    ...
    public function cancle_order(){
        $postdata = $this->input->post();
        if(isset($postdata['order_id'])){
            $this->this_model->cancle_order($postdata);
    ...
    public function product_search()
    {
        ...
        if(isset($product_name) && $product_name != '' && isset($vendor_id) && $vendor_id != ''){
            ...
            // $query = $this->db->query("SELECT p.* FROM `product` as p 
            //                         LEFT JOIN product_weight as w ON w.product_id = p.id
            //                         WHERE p.status != '9' AND p.vendor_id = '$vendor_id' AND w.discount_price != '' AND w.status != '9' AND p.name LIKE '%$product_name%' GROUP BY p.id ORDER BY CAST(w.discount_price AS DECIMAL(10,2)) ASC ");
...
public function make_payment(){
    $this->this_model->make_payment($this->input->post());  
}
...
public function sendMailSMTP($data) {
    ...
    $config ['smtp_user'] = "shakti.cmexpertise@gmail.com";
    $config ['smtp_pass'] = "Shakti@wo`123";

...
class Api_admin extends Api_Controller {
    ...
    */
    ...
    public function staff_add() {
        $post = $this->input->post();
        $req = array('vendor_id', 'name', 'email', 'password', 'mobile', 'vehicle_number', 'vehicle_name');
        $required = $this->checkRequiredField($post, $req);
        if ($required['status'] == 1) {
    ...
    public function staff_update() {
        $post = $this->input->post();
        $req = array('staff_id', 'vendor_id', 'name', 'mobile', 'vehicle_number', 'vehicle_name');
        $required = $this->checkRequiredField($post, $req);
        if ($required['status'] == 1) {
            $responce = $this->this_model->staff_add_update($this->input->post());
        }
    ...
    public function product_add() {
        $post = $this->input->post();
        $req = array('vendor_id', 'name', 'category_id', 'brand_id', 'subcategory_id', 'about', 'content');
        $required = $this->checkRequiredField($post, $req);
        if ($required['status'] == 1) {
    ...
    public function product_update() {
        $post = $this->input->post();
        $req = array('vendor_id', 'name', 'category_id', 'brand_id', 'subcategory_id', 'about', 'content', 'product_id');
        $required = $this->checkRequiredField($post, $req);
        if ($required['status'] == 1) {
            $responce = $this->this_model->product_add_update($this->input->post());
        }

...
Class Migration_database_new_model extends My_model{
...
// 			if ($this->db->field_exists('vendor_id', "a1630btr_dev .".$value))
// 			{
// 				if($value != 'admin' && $value != 'vendor'){
// 					// $this->db->query('ALTER TABLE '."a1630btr_dev .".$value.' ADD vendor_id int(11) NOT NULL AFTER id');
// 					$this->db->query('ALTER TABLE '."a1630btr_dev .".$value.' CHANGE COLUMN vendor_id branch_id int(11)');
// 				}
// 			}
// 		}	
// die;	
...
echo 'please remove die';die;
		...
		$server_name = ['bmart.launchestore.com','iwingzy.launchestore.com','shukantea.launchestore.com','laxmiraj.launchestore.com','iwingzy.launchestore.com','ori.launchestore.com','ogworld.launchestore.com','laxmiraj.launchestore.com','bigbucket.launchestore.com','shukantea.launchestore.com','ori.launchestore.com','bigbucket.launchestore.com','laxmirajdryfruit.com','shukantea.com','ugiftonline.com','unibest.launchstore.com','rready.launchestore.com','ogworld.launchestore.com','mehta.launchestore.com','magnus.launchestore.com','laxmiraj.launchestore.com','wingzy.launchestore.com','bigbucket.launchestore.com','shukantea.launchestore.com'];
		...
		foreach ($db as $key => $database) {
			foreach ($array  as $k => $table) {
					...
					foreach ($re as $keys => $value) {
						...
						$data['table'] = 'a1630btr_dev.vendor';
						$data['select'] = ['id'];
						$data['order'] = 'id desc';
						$data['limit'] = '1';
						...
						$data['table'] = 'a1630btr_dev.branch';
						$data['select'] = ['id'];
						$data['order'] = 'id desc';
						$data['limit'] = '1';
						...
						if(($table == 'subcategory')){
							...
							// 	print_r($name);die;
							// }
							$category_id = $this->getIdByname($value['branch_id'],'a1630btr_dev',$name[0]->name,'category','name');
						...
						if($table == 'brand'){
							...
							foreach (explode(',', $brand[0]->category_id) as $b => $v) {
								...
								if(!empty($r)){
									$catId = $this->getIdByname($value['branch_id'],'a1630btr_dev',$r[0]->name,'category','name');
								}
						...
						if($table == 'user_address'){
							...
							if(!empty($record)){
								if($record[0]->email != ''){
									$user_id = $this->getIdByname($value['vendor_id'],'a1630btr_dev',$record[0]->email,'user','email');
								}else{
									$user_id = $this->getIdByname($value['vendor_id'],'a1630btr_dev',$record[0]->apple_token_id,'user','apple_token_id');
								}
							}else{
								$user_id = 0;
							}
						...
						if($table == 'customer'){
							...
							$group_id = $this->getIdByname($value['branch_id'],'a1630btr_dev',$customerData[0]->name,'customer_group','name');
						...
						if($table == 'delivery_user_device'){
							...
							$delivery_user_id = $this->getIdByname($value['vendor_id'],'a1630btr_dev',$deliveryData[0]->email,'delivery_user','email');
						...
						if($table == 'staff_device'){
							...
							// print_r($productData);die;
							$staff_id = $this->getIdByname('','a1630btr_dev',$staffData[0]->email,'staff','email');
						...
						if($table == 'banner_promotion'){
							...
							$product_id = $this->getIdByname($value['branch_id'],'a1630btr_dev',$productData[0]->name,'product','name');
							// echo $this->db->last_query();
							// print_r($productData);die;
						...
						if($table == 'product_weight'){
							...
							$weight_id = $this->getIdByname($value['vendor_id'],'a1630btr_dev',$weightData[0]->name,'weight','name');
						
							$packageData = $this->getTableIdByforignKey('',$database,$value['package'],'package','id');

							$package_id = $this->getIdByname($value['vendor_id'],'a1630btr_dev',$packageData[0]->package,'package','package');
						...
						// 	$data['table'] = 'a1630btr_dev.product as p';
						// 	$data['select'] = ['pw.id','p.name','pw.weight_id','pw.package','pw.weight_no','pw.dt_added'];
						// 	$data['join'] = [
						// 		'a1630btr_dev.product_weight as pw'=>['p.id=pw.product_id','LEFT']
						// 	];
						...
						// 	// $varient_id = $this->getDataByJoin($value['vendor_id'],'a1630btr_dev',$varientData[0]->name,'product','product_weight','name');
						// 	$value['product_id'] = $product_id;	
						...
						if($table == 'product_image'){
							...
							$data['table'] = 'a1630btr_dev.product_weight';
							$varient_id = $this->selectFromJoin($data);
							if(empty($varient_id)){		
						...
						if($table == 'product'){
							...
							$category_id = $this->getIdByname($value['branch_id'],'a1630btr_dev',$cateData[0]->name,'category','name');

							
							$subData = $this->getTableIdByforignKey('',$database,$value['subcategory_id'],'subcategory','id');

							$subcategory_id = $this->getIdByname($value['branch_id'],'a1630btr_dev',$subData[0]->name,'subcategory','name');
							
							$value['subcategory_id'] = $subcategory_id;
						
							$brandData = $this->getTableIdByforignKey('',$database,$value['brand_id'],'brand','id');
							$brand_id = $this->getIdByname($value['branch_id'],'a1630btr_dev',$brandData[0]->name,'brand','name');
						...
						if($table == 'my_cart'){
							...
							if(!empty($userData)){
								if($userData[0]->email != ''){
									$user_id = $this->getIdByname($value['vendor_id'],'a1630btr_dev',$userData[0]->email,'user','email');
								}else{
									$user_id = $this->getIdByname($value['vendor_id'],'a1630btr_dev',$userData[0]->apple_token_id,'user','apple_token_id');
								}
							}else{
								$user_id = 0;
							}
							...
							$varient_id = $this->getDataByJoin($value['branch_id'],'a1630btr_dev',$varientData[0]->name,'product','product_weight','name');
						...
						if($table == 'order'){
							...
							if($value['customer_id'] != 0){
								$customer_id = $this->getIdByname($value['branch_id'],'a1630btr_dev',$customerData[0]->customer_name,'customer','customer_name');
							}else{
								$customer_id = '';
							}
							...
							$UserAddress = $this->getUserDataByAddressId('','a1630btr_dev',$user_address_id,'user','user_address','id');
							if(empty($UserAddress)){
								$user_address_id = '0';
							}else{
								$user_address_id = $UserAddress[0]->id;
							...
							if($value['time_slot_id']!=0){
								...
								$TimeSlot = $this->getTableIdByforignKey('',$database,$value['time_slot_id'],'time_slot','id');
							 	$time_slot_id = $this->getTimeSlotIdByname($value['vendor_id'],'a1630btr_dev',$TimeSlot[0]->start_time,$TimeSlot[0]->end_time);
							...
							foreach ($order_details as $order_detail_value) {
								...
								$user_id = $this->getIdByname($order_detail_value['vendor_id'],'a1630btr_dev',$userData[0]->email,'user','email');

								$productData = $this->getTableIdByforignKey('',$database,$order_detail_value['product_id'],'product','id');

								$product_id = $this->getIdByname($order_detail_value['branch_id'],'a1630btr_dev',$productData[0]->name,'product','name');								

								$varientData = $this->getDataByJoin('',$database,$order_detail_value['product_weight_id'],'product','product_weight','id');

								$varient_id = $this->getDataByJoin($order_detail_value['branch_id'],'a1630btr_dev',$varientData[0]->name,'product','product_weight','name');

								$weightData = $this->getTableIdByforignKey('',$database,$order_detail_value['weight_id'],'weight','id');
							
								$weight_id = $this->getIdByname($order_detail_value['vendor_id'],'a1630btr_dev',$weightData[0]->name,'weight','name');
								...
								// $data['table'] = 'a1630btr_dev.order_details';
								$data['table'] = 'a1630btr_dev.order_details';
						...
						if($table == 'delivery_order'){
							...
							$order_id = $this->getIdByname($value['branch_id'],'a1630btr_dev',$Data[0]->dt_added,'order','dt_added');
							unset($Data);

							$Data = $this->getTableIdByforignKey('',$database,$value['delivery_user_id'],'delivery_user','id');

							$delivery_user_id = $this->getIdByname($value['branch_id'],'a1630btr_dev',$Data[0]->dt_added,'delivery_user','dt_added');
						...
						if($table == 'notification'){
							...
							$user_id = $this->getIdByname($value['vendor_id'],'a1630btr_dev',$Data[0]->email,'user','email');
							// echo $this->db->last_query();
							if($order_id != 0){
						...
						if($table == 'selfPickup_otp'){
							...
							$user_id = $this->getIdByname($value['vendor_id'],'a1630btr_dev',trim($Data[0]->email),'user','email');
							if($order_id != 0){
								$value['order_id'] = $order_id;
							}
						...
						// 	$Data = $this->getTableIdByforignKey('',$database,$value['order_id'],'order','id');
						// 	if(!empty($Data)){
						// 		$order_id = $this->getIdByname($value['vendor_id'],'a1630btr_dev',$Data[0]->dt_added,'order','dt_added');
						// 		$value['order_id'] = $order_id;
						// 	}
	...
	function getTableIdByforignKey($vendor_id = '',$database,$param,$table,$field){
		...
		if($table == 'vendor' && $database == 'a1630btr_dev'){
			$table = 'branch';
		}
		if($field == 'vendor_id' && $database == 'a1630btr_dev'){
			$field = 'branch_id';
		}
		if( $table == 'about_section_one' || $table == 'about_section_two' || $table == 'about_us' ||
			...
			$table == 'user' && ($database == 'a1630btr_dev')){
	...
	function getDataByJoin($vendor_id = '',$database,$param,$table1,$table2,$field){
		...
		if($table1 == 'vendor' && $database == 'a1630btr_dev'){

...
class Delivery_api_model extends My_model
{
    public function __construct() {
         ...
         $data = $this->common_model->getCommonKeysAndLink();
         $this->url =$data[0]->firebase_url; 
         $this->token = $data[0]->firebase_token;
         $this->key = $data[0]->delivery_firebase_key;
         $this->firebaseNode = $data[0]->firebase_node;
    ...
    public function send_notification($order_id){
        ...
        $data['select'] = ['o.order_no','v.name','ua.address as b_address','ua.latitude as b_lat','ua.longitude as b_long','v.location as s_address','v.latitude as s_lat','v.longitude as s_long','o.isSelfPickup'];
        $data['join'] = [
        ...
        foreach ($decodeRe as $value) {
            ...
            if($value->currentStatus=='1' && $distance <=50000000000000000000000000000){
                ...
                // $this->load->model('common_model');
                // $keys = $this->common_model->getCommonKeysAndLink();
                // if($keys[0]->delivery_firebase_key != ""){
                //     $key = $keys[0]->delivery_firebase_key;
                // }



                    // $this->utility->sendNotification($dataArray, $notification_type,NULL,$key);
                    $this->utility->sendNotification($dataArray, $notification_type,NULL,$this->key);
                    // echo 1; exit;

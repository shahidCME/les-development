...
class staff_api_model extends my_model {
    function login($postdata) {
        ...
        $data['select'] = ['s.id', 's.vendor_id', 's.name', 's.email', 's.email_verify', 's.email_token', 's.phone_no', 'v.name AS shopname', 'v.location', 's.status', 'v.delivery_by','v.status as vendor_status'];
    ...
    function order_list($postdata) {
        ...
        $data['where'] = ['o.vendor_id' => $vendor_id, 'o.status !=' => '9','pi.status !=' => '9'];

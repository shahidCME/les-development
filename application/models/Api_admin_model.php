...
class Api_admin_model extends My_model {
    public function check_login($postData) {
        ...
        if($row_login['status'] != '0'){
            if ($result_login->num_rows() > 0) {
                ...
                if ($status == '1') {
                    $login_data = array('id' => $row_login['id'], 'name' => $row_login['owner_name'], 'email' => $row_login['email'], 'phone' => $row_login['phone_no'], 'selfPickUp' => $row_login['selfPickUp'], 'shopName' => $row_login['name'], 'token' => $token, 'gst_number' => $row_login['gst_number'], 'image' => base_url() . 'public/images/'.$this->folder.'vendor_shop/' . $row_login['image'],'currancy'=>$row_default['value'],'address'=>$row_login['address'] ,'logged_in' => TRUE);
     ...
     public function is_authenticate($postData) {
        ...
        $result_login = $this->db->query("SELECT * FROM vendor WHERE id='$vendor_id'");
        $row_login = $result_login->row_array();
        ...
        if($row_login['status'] != '0'){
            if ($result_login->num_rows() > 0) {
                ...
                $status = $row_login['status'];
                if ($status == '1') {
                     $login_data = array('id' => $row_login['id'], 'name' => $row_login['owner_name'], 'email' => $row_login['email'], 'phone' => $row_login['phone_no'], 'selfPickUp' => $row_login['selfPickUp'], 'shopName' => $row_login['name'], 'token' => $row_login['token'], 'gst_number' => $row_login['gst_number'], 'image' => base_url() . 'public/images/'.$this->folder.'vendor_shop/' . $row_login['image'],'currancy'=>$row_default['value'],'address'=>$row_login['address'] ,'logged_in' => TRUE);
                    $res = ['status' => 1, 'message' => 'Data get success', 'data' => $login_data];
                } else {
                    $res = ['status' => 0, 'message' => 'User inactivated by admin'];
                }
    ...
    public function profile_update($postData) {
        ...
        // print_r($_FILES);die;
        $vendor_id = $postData['vendor_id'];
        $query = $this->db->query("SELECT image,token,gst_number,address FROM vendor WHERE id = '$vendor_id'");
    ...
    public function add_group($postData) {
        $vendor_id = $postData['vendor_id'];
        $name = $postData['name'];
        $type_array = array('name' => $name, 'vendor_id' => $vendor_id, 'status' => '1', 'dt_added' => strtotime(DATE_TIME), 'dt_updated' => strtotime(DATE_TIME));
    ...
    public function staff_add_update($postData) {
        ...
        if (empty($res)) {
            if (isset($postData['staff_id']) && $postData['staff_id'] != '') {
                $dataupdate = array('vendor_id' => $vendor_id, 'name' => $postData['name'], 'phone_no' => $postData['mobile'], 'vehicle_number' => $postData['vehicle_number'], 'vehicle_name' => $postData['vehicle_name'], 'dt_added' => DATE_TIME, 'dt_updated' => DATE_TIME,);
    ...
    public function category_add_update($postData) {
        ...
        ## Add Category ##
            ...
            $uploadResponse['data']['file_name'];
            $this->image_resize_category($uploadResponse['data']['full_path'], $uploadResponse['data']['file_name']);
            $data = array('vendor_id' => $vendor_id, 'name' => $name, 'image' => $uploadResponse['data']['file_name'], 'status' => '1', 'dt_added' => strtotime(DATE_TIME), 'dt_updated' => strtotime(DATE_TIME));
    ...
    public function get_subcategory($postData) {
        ...
        $data['where'] = ['s.vendor_id' => $postData['vendor_id'], 's.  status !=' => '9'];
        $data['join'] = [TABLE_CATEGORY.' as c' => ['c.id = s.category_id', 'LEFT']];
        $data['order'] = 's.id DESC';
        $data['select'] = ['s.*', 'c.name as category_name'];
        $data['table'] = TABLE_SUBCATEGORY.' as s';
    ...
    public function subCategory_add_update($postData) {
        ...
        if (isset($postData['subCategory_id']) && $postData['subCategory_id'] != '') {
            ...
            foreach ($name as $key => $value) {
                $data = array('vendor_id' => $vendor_id, 'name' => $value->name, 'category_id' => $category_id, 'status' => '1', 'dt_added' => strtotime(DATE_TIME), 'dt_updated' => strtotime(DATE_TIME),);
    ...
    public function brand_add_update($postData) {
        ...
        /* Brand Add */
            ...
            $is_available = $this->selectRecords($data);
            if (empty($is_available)) {
                $data = array('vendor_id' => $vendor_id, 'name' => $name, 'category_id' => $cate_id, 'status' => '1', 'dt_added' => strtotime(DATE_TIME), 'dt_updated' => strtotime(DATE_TIME),);
    ...
    public function product_add_update($postData) {
        ...
        ## Add Product ##
            ...
            $data = array('vendor_id' => $vendor_id, 'category_id' => $category_id, 'brand_id' => $brand_id, 'subcategory_id' => $subcategory_id, 'supplier_id' => $supplier_id, 'name' => $name, 'about' => $about, 'content' => $content, 'status' => '1', 'gst' => $gst, 'dt_added' => strtotime(DATE_TIME), 'dt_updated' => strtotime(DATE_TIME));
    ...
    public function product_weight_add_update($postData) {
        ...
        /* Product Weight Add */
            ...
            $data = array('vendor_id' => $vendor_id, 'product_id' => $product_id, 'weight_id' => $weight_id, 'weight_no' => $unit, 'package' => $package, 'purchase_price' => $purchase_price, 'price' => $price, 'quantity' => $quantity, 'discount_per' => $discount_per, 'discount_price' => $final_discount_price, 'status' => '1', 'dt_added' => strtotime(DATE_TIME), 'dt_updated' => strtotime(DATE_TIME),);
        ...
        if ($file['userfile']['name'][0] != '') {
            ...
            if ($file['userfile']['name'][0] != '') {
                ...
                for ($i = 0;$i < $cpt;$i++) {
                    ...
                    $data = array('image_order' => '0', 'vendor_id' => $vendor_id, 'image' => $file['userfile']['name'], 'product_id' => $postData['product_id'], 'product_variant_id' => $variant_id, 'status' => '1', 'dt_added' => strtotime(DATE_TIME), 'dt_updated' => strtotime(DATE_TIME));
